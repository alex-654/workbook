...
не опирающимися на какие-либо инфраструктурные компоненты или платформы и не включающими их напрямую.

#### Объект-значение

Использовать объекты-значения при любой возможности.
Объекты-значения не только делают код более выразительным и инкапсулируют бизнес-логику, имеющую свойство дублироваться
в разных местах, но и само применение данного паттерна проектирования делает код более безопасным.
...
- В состав агрегата должна входить только та информация,
...
Со временем можно столкнуться с бизнес-логикой, которая

- либо не принадлежит ни одному агрегату или объекту-значению,
- либо представляется имеющей отношение сразу к нескольким агрегатам.

В таких случаях предметно-ориентированное
...
- Доменный сервис — это объект без состояния, в котором реализуется бизнес-логика.
  Обычно такая логика является организатором обращений к различным компонентам системы для выполнения
  вычислений/анализа.
...
- Паттерны агрегатов и объектов-значений: инкапсулируют инварианты, уменьшая таким образом сложность.
...
- действует как посредник между слоями представления и бизнес-логики.
- логика оркестрации -> сервисный слой
- Сервисный слой выступает в качестве фасада слоя бизнес-логики: инкапсулируя вызовы нижележащих слоев.
...
Необходимость в сервисном слое возникает не всегда.
Например, когда бизнес-логика реализована в виде транзакционного сценария,
то она, по сути, и представляет собой сервисный слой, поскольку уже предоставляет набор методов, 
формирующих публичный интерфейс системы. 
В таком случае API сервисного слоя будет просто повторять публичные интерфейсы транзакционных
...
подчиняется принципу согласованности (итоговая согласованность, eventual consistency).
И хотя сага в конечном итоге выполнит соответствующие команды, никакие две транзакции нельзя считать атомарными. 
Это соотносится с еще одним принципом построения агрегатов:
